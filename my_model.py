...
from allennlp_models.structured_prediction.metrics.srl_eval_scorer import DEFAULT_SRL_EVAL_PATH, SrlEvalScorer

# This file is almost identical to allennlp_models/syntax/srl/srl_bert_model.py
# But the original file couldn't be used with xlmr models (at the time)

@Model.register("my_model")
class SrlMyModel(Model):
    """
    ...
    def forward(  # type: ignore
        ...
        if tags is not None:
            ...
            if not self.ignore_span_metric and self.span_metric is not None and not self.training:
                ...
                if self.file_path is not None:
                    f.writelines("%s\t%s\t%s\n" % (s,x1,x2) for i in range(len(batch_conll_predicted_tags)) \
                        for s,x1,x2 in zip(list(words)[i] + ["-"], batch_conll_gold_tags[i]+["-"], batch_conll_predicted_tags[i]+["-"]))
                self.span_metric(
                    batch_verb_indices,
                    batch_sentences,
                    batch_conll_predicted_tags,
